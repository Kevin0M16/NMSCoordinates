...
namespace NMSCoordinates
{
    public class Coordinates
    {
        ...
        public static Destination TeleportEndpoints(int i, string json, TextBox tb = null)
        {
            ...
            return BuildDestinationFromEndpoint(endpoint, tb);
        }

        private static Destination BuildDestinationFromEndpoint(dynamic endpoint, TextBox tb = null)
        {
            Destination dest = new Destination
            {
                Galaxy = endpoint.UniverseAddress.RealityIndex.ToString(),
                X = endpoint.UniverseAddress.GalacticAddress.VoxelX.ToString(),
                Y = endpoint.UniverseAddress.GalacticAddress.VoxelY.ToString(),
                Z = endpoint.UniverseAddress.GalacticAddress.VoxelZ.ToString(),
                SSI = endpoint.UniverseAddress.GalacticAddress.SolarSystemIndex.ToString(),
                PI = endpoint.UniverseAddress.GalacticAddress.PlanetIndex.ToString(),

                iGalaxy = Convert.ToInt32(endpoint.UniverseAddress.RealityIndex),
                iX = Convert.ToInt32(endpoint.UniverseAddress.GalacticAddress.VoxelX),
                iY = Convert.ToInt32(endpoint.UniverseAddress.GalacticAddress.VoxelY),
                iZ = Convert.ToInt32(endpoint.UniverseAddress.GalacticAddress.VoxelZ),
                iSSI = Convert.ToInt32(endpoint.UniverseAddress.GalacticAddress.SolarSystemIndex),
                iPI = Convert.ToInt32(endpoint.UniverseAddress.GalacticAddress.PlanetIndex)
            };
        ...
        {
            dynamic nms = SaveFileParser.ParseSaveData(json);
            string basehex = Globals.IsNewSaveFormat
                ? nms.BaseContext.PlayerStateData.PersistentPlayerBases[i].GalacticAddress
                : nms.PlayerStateData.PersistentPlayerBases[i].GalacticAddress;

            return HexToAll(basehex);
        }

        public static Player PlayerGalaxy(string json)
        {
            dynamic nms = SaveFileParser.ParseSaveData(json);
            dynamic playerState = Globals.IsNewSaveFormat
                ? nms.BaseContext.PlayerStateData
                : nms.PlayerStateData;

            return BuildPlayerFromState(playerState);
        }
        ...
        private static Player BuildPlayerFromState(dynamic playerState)
        {
            Player player = new Player
            {
                Galaxy = playerState.UniverseAddress.RealityIndex.ToString(),
                X = playerState.UniverseAddress.GalacticAddress.VoxelX.ToString(),
                Y = playerState.UniverseAddress.GalacticAddress.VoxelY.ToString(),
                Z = playerState.UniverseAddress.GalacticAddress.VoxelZ.ToString(),
                SSI = playerState.UniverseAddress.GalacticAddress.SolarSystemIndex.ToString(),
                PI = playerState.UniverseAddress.GalacticAddress.PlanetIndex.ToString(),

                iGalaxy = Convert.ToInt32(playerState.UniverseAddress.RealityIndex),
                iX = Convert.ToInt32(playerState.UniverseAddress.GalacticAddress.VoxelX),
                iY = Convert.ToInt32(playerState.UniverseAddress.GalacticAddress.VoxelY),
                iZ = Convert.ToInt32(playerState.UniverseAddress.GalacticAddress.VoxelZ),
                iSSI = Convert.ToInt32(playerState.UniverseAddress.GalacticAddress.SolarSystemIndex),
                iPI = Convert.ToInt32(playerState.UniverseAddress.GalacticAddress.PlanetIndex)
            };
        ...
        public static Destination GalacticToVoxel(int galaxy, string X, string Y, string Z, string SSI, TextBox tb = null)
        {
        ...
        public static Destination HexToAll(string basehx, TextBox tb = null)
        {
